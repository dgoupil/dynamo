...
func TestDynamoComponentDeploymentReconciler_generatePodTemplateSpec_RestoreLabels(t *testing.T) {
			...
			Spec: v1alpha1.DynamoComponentDeploymentSpec{
				...
				DynamoComponentDeploymentSharedSpec: v1alpha1.DynamoComponentDeploymentSharedSpec{
					...
					Labels: map[string]string{
						...
						commonconsts.KubeLabelRestoreRequestID:          "stale-request-id",
					},
	...
	t.Run("ready checkpoint adds explicit restore labels", func(t *testing.T) {
		...
		if got := podTemplateSpec.Labels[commonconsts.KubeLabelRestoreRequestID]; got != "hash-ready-1" {
			t.Fatalf("expected %s to be checkpoint hash, got %q", commonconsts.KubeLabelRestoreRequestID, got)
		}
	})
	...
	t.Run("non-ready checkpoint clears stale restore labels", func(t *testing.T) {
		...
		if _, ok := podTemplateSpec.Labels[commonconsts.KubeLabelRestoreRequestID]; ok {
			t.Fatalf("did not expect %s label when checkpoint is not ready", commonconsts.KubeLabelRestoreRequestID)
		}
	})
}
